...
b)  Exponentiate the coefficients. Interpret the odds ratio for `Age` in one sentence.
    ...
    When age increases 1 year, the odds of being in a higher rank increase 1.0056%(?).
...
c)  Using Model 2, what is the predicted probability of Rank 1 for a 30-year-old female? Use `predictions()` from the `marginaleffects` package.
    ...
    34.6%. Have we learned how to use this function..?
...
predictions(m2, newdata = data.frame(Gender = "F", Age = 30))
```
...
modelsummary(models, exponentiate = TRUE, stars = TRUE)
```
...
All p values are not significant, so the assumption appears to hold.
