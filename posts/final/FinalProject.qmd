...
Let's take a quick look at our data here. We're using `ggside:::geom_xsidehistogram` and `ggside:::geom_ysidehistogram` to add marginal histograms to a scatter plot. We can see that the similarity rating distributes from 3 to 7.
...
Before diving into Bayesian modeling, let’s warm up with the familiar `lm()` function. This gives us a useful baseline and reminds us of the basic regression setup: we are predicting perceived similarity, `mean_similarity_comp`, from standardized neural alignment, `r2_vox_z`.
...
Since `mean_similarity_comp` is measured on a 1–7 scale, I set the intercept prior to be centered around 4.5. I use a skeptical prior centered at 0 for the slope. Finally, I use a lognormal prior for `sigma`, the residual standard deviation, because `sigma` must be positive.
...
After adding the interaction between neural alignment and dyad group, the model suggested a clear group difference but weak evidence for different slopes by group. - Friend dyads showed an estimated positive slope for `r2_vox_z` of 0.22, but the 95% credible interval ranged from -0.03 to 0.47. - Stranger dyads were estimated to report similarity scores 1.51 points lower than friend dyads when neural alignment was average. - The interaction term was -0.22, with a 95% credible interval from -0.55 to 0.12, suggesting that the data do not provide strong evidence that the effect differs between friend and stranger dyads.
...
The interaction model suggests that the neural alignment slope may be more positive for friend dyads than for stranger dyads. Because both intervals include 0, the model does not provide strong evidence that neural alignment clearly predicts perceived similarity within either group.
